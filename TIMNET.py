...
def Temporal_Aware_Block(x, s, i, activation, nb_filters, kernel_size, dropout_rate=0, name=''):
    ...
    conv_1_1 = BatchNormalization(axis=-1)(conv_1_1)
    ...
    conv_2_1 = BatchNormalization(axis=-1)(conv_2_1)
    ...
    def __call__(self, inputs, mask=None):
        ...
        for s in range(self.nb_stacks):
            for i in [2 ** i for i in range(self.dilations)]:
                ...
                temp_skip = tf.keras.layers.Add()([skip_out_forward, skip_out_backward])
        ...
        for i, item in enumerate(final_skip_connection):
            if i == 0:
                continue
            output_2 = tf.keras.layers.Concatenate(axis=-2)([output_2, item])
        x = output_2
